...
def hsnr_preprocessing(df: pd.DataFrame, column: str) -> pd.DataFrame:
    ...
    df[column_clean] = df[column_clean].str.replace(pattern_extra_zero, r'\2', regex=True)
    ...
    df[column_clean] = np.where(
        df[column_clean].str.contains(
            r"(?i)\s*unbekannt\s*", regex=True, na=False
        ),
...
def extract_multiple_numbers_sets(s):
    return list(map(int, re.findall(r'\d+', s)[1:])) if len(re.findall(r'\d+', s)) > 1 else []
...
def compare_components(comp1, comp2):
    ...
    elif (comp1 and comp2 ) and (set(comp1).issubset(set(comp2)) or set(comp2).issubset(set(comp1))):
        return 0.75
...
def calculate_similarity(row) :
    ...
    numbers_score = (
        compare_components(row["HSNR_FIRST_1"], row["HSNR_FIRST_2"]) *.8 +
        compare_components2(row["HSNR_RANGE_1"], row["HSNR_RANGE_2"])*.2 
...
def compare_house_numbers( df:pd.DataFrame, column1:str, column2:str , threshold:float=None ):
    ...
    df["HSNR_LETTER_1"] = np.where(
        (df[column1].str.contains(r'[a-zA-ZäÄöÖüÜß]{1,2}', regex=True) &  (df["HSNR_WORD_1"].apply(len) == 0)),
    ...
    df["HSNR_LETTER_2"] = np.where(
        (df[column2].str.contains(r'[a-zA-ZäÄöÖüÜß]{1,2}', regex=True) &  (df["HSNR_WORD_2"].apply(len) == 0)),
    ...
    df.drop(columns =['HSNR_DF1_CLEAN', 'HSNR_DF2_CLEAN', 'HSNR_FIRST_1', 'HSNR_RANGE_1',
       'HSNR_LETTER_1', 'HSNR_WORD_1', 'HSNR_FIRST_2', 'HSNR_RANGE_2',
       'HSNR_LETTER_2', 'HSNR_WORD_2'] ,inplace = True)
